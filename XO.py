...
def clear_session(server, nickname, enemy_nickname):
    ...
    players_file_lines.pop(max(self_index, enemy_index))
    players_file_lines.pop(min(self_index, enemy_index))
    players_file.close()
...
def create_session(server, nickname, enemy_nickname):
    ...
    if directory != []:
        ...
        players_numbers = [int(i.split()[-1]) for i in filter(lambda x: x.split()[-1] in [str(i) for i in range(10)], players_file_lines)]
        ...
        if len(players_file_lines) > 1:
            ...
            for i in range(len(players_file_lines)):
                if i != self_index and players_file_states[i] == 'waiting':
                    ...
                    players_file_lines[self_index] = nickname + ' confirming\n'
                    found = True
                    break
            if not found:
